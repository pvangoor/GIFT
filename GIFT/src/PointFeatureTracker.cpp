...
vector<Landmark> PointFeatureTracker::createNewLandmarks(const Mat& image, const vector<Point2f>& newFeatures) {
    ...
    cv::undistortPoints(newFeatures, newFeaturesNorm, cameraPtr->K, cameraPtr->distortionParams);
...
void PointFeatureTracker::trackLandmarks(const Mat& image) {
    ...
    cv::undistortPoints(points, pointsNorm, cameraPtr->K, cameraPtr->distortionParams);
