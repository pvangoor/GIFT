...
Mat GIFT::drawFlowImage(const Mat& image0, const Mat& image1, const vector<Landmark>& landmarks0, const vector<Landmark>& landmarks1, const int& radius, const Scalar& circleColor, const int& thickness, const Scalar& lineColor) {
    // draw the flow image on a red/blue merge of image0 and image1
    Mat gray0 = image0; if (image0.channels() > 1) cvtColor(image0, gray0, COLOR_BGR2GRAY);
    Mat gray1 = image1; if (image1.channels() > 1) cvtColor(image1, gray1, COLOR_BGR2GRAY);
