...
protected:
    EgoMotionTest(int pointCount = 100) {
        ...
        for (int i=0; i<pointCount; ++i) {
            ...
            while (point.norm() < 1e-1) point = Vector3T::Random()*100;
...
TEST_F(EgoMotionTest, ConvergesLinVel) {
    ...
    for (int i = 0; i < testCount; ++i) {
        ...
        for (const auto& etaRho: bearingsAndInvDepths) {
            Vector3T phi = etaRho.second * (Matrix3T::Identity() - etaRho.first*etaRho.first.transpose()) * trueLinVel - trueAngVel.cross(etaRho.first);
...
TEST_F(EgoMotionTest, ConvergesAngVelFromLinVel) {
    ...
    for (int i = 0; i < testCount; ++i) {
        ...
        Vector3T initialLinVel = (trueLinVel + i*0.25*trueLinVel.norm()*Vector3T::Random()/testCount).normalized();
